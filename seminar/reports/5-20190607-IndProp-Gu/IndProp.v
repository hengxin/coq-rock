...
Proof.
  ...
  destruct H as [ HL | [m [HL HR]]].
  - discriminate HL.
  - injection HL. intros HEQ. rewrite <- HEQ in HR. apply HR.
Qed. 
*)
...
Proof.
  ...
  destruct H as [HL | [m [HL HR]]].
  - discriminate HL.
  - apply ev_inversion in HR. injection HL as HL.
    destruct HR as [H1 | [k [H1 H2]]].
    + rewrite -> H1 in HL. discriminate HL.
    + rewrite H1 in HL. injection HL as HL.
      rewrite HL. apply H2.
Qed.
...
Theorem even5_nonsense :
  even 5 -> 2 + 2 = 9.
Proof.
  intros H. inversion H as [|n E]. inversion E as [|n' E'].
  inversion E'. 
Qed.
...
(*
Proof.
  intros n H. induction H as [|n' E' IHn'].
  - exists 0. reflexivity.
...
Proof.
  intros n m H1 H2. generalize dependent m.
  induction H1 as [|n' E' IHn'].
  - intros m. simpl. intros H. apply H.
  - intros m. intros H1. 
    assert(H : S (S n') + m = n' + (1 + 1 + m)).
    + rewrite <- plus_1_l. rewrite -> (plus_comm 1 (S n')). rewrite <- plus_1_l.
      rewrite -> (plus_comm 1 n'). symmetry. rewrite -> plus_assoc. 
      rewrite -> plus_assoc. reflexivity.
    + rewrite -> H. apply IHn'. simpl. apply (ev_SS m H1).
Qed.
...
Proof.
  intros n. split.
  - intros H. induction H as [| |n' m' En' IHn' Em' IHm'].
    + apply ev_0.
    + apply (ev_SS 0 (ev_0)).
    + apply ev_sum. apply IHn'. apply IHm'.
  - intros H. induction H as [|n' E' IHn'].
    + apply even'_0.
    + rewrite <- plus_1_l. rewrite -> plus_comm. rewrite <- plus_1_l. 
      rewrite -> (plus_comm 1 n'). rewrite <- plus_assoc. simpl.
...
Proof.
  ...
  - intros m. intros H1. simpl in H1. apply H1.
  - intros m. intros H1. 
    rewrite <- plus_1_l in H1. rewrite -> (plus_comm 1 (S n')) in H1.
    rewrite <- plus_1_l in H1. rewrite -> (plus_comm 1 n') in H1.
    rewrite <- plus_assoc in H1. rewrite <- plus_assoc in H1.
    apply IHn' in H1. simpl in H1. apply evSS_ev in H1. apply H1.
Qed.
...
Proof.
  ...
  - rewrite -> plus_assoc in H. rewrite -> (plus_comm (n+m) n) in H.
    rewrite -> plus_assoc in H. rewrite <- plus_assoc in H.
    assert(H3 : even (n+n)).
    + rewrite <- double_plus. apply ev_double.
    + apply (ev_ev__ev (n+n) (m+p) H H3).
Qed.
...
Inductive total_rel : (prod nat nat) -> (prod nat nat) -> Prop :=
  |cp_1 (x11 x12 x21 x22 : nat) (H: lt x11 x21) : total_rel (x11,x12) (x21,x22)
  |cp_2 (x11 x12 x21 x22 : nat) (H: le x11 x21) : total_rel (x11,x12) (x21,x22).
...
Proof.
  ...
  induction H2 as [|o' E' IHo'].
  - intros m. intros H. apply H.
  - intros m. intros H3. apply (le_S m o'). 
    apply IHo' in H3. apply H3.
Qed.
...
Proof.
  intros n m H.
  induction H as [|m' E' IHm'].
  - apply (le_n (S n)).
  - apply (le_S (S n) (S m') IHm').
Qed.  
...
Lemma zero_lt_any : forall n, 0 <= n.
Proof.
...
(*  ATTENTIN:!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  why cannot use destruct or induction?
...
Theorem Sn_le_Sm__n_le_m : forall n m,
  S n <= S m -> n <= m.
Proof.
  intros n m H. inversion H as [|m' E].
  - apply (le_n m).
  - assert(H1 : n <= S n).
    + apply (le_S n n (le_n n)).
    + apply (le_trans n (S n) m H1 E).
Qed.

Search plus.
Search S.

Lemma plus_1_test : forall n m, n + S m = S (n + m).
Proof.
  Admitted.
...
Proof.
  ...
  - rewrite <- plus_n_O. apply (le_n a).
  - assert(H : a + b' <= S (a + b')).
    + apply (le_S (a+b') (a+b') (le_n (a+b'))).
    + rewrite <- plus_1_test in H.
      apply (le_trans a (a+b') (a+(S b')) IHb' H).
Qed. 
...
Proof.
 ...
 - apply (le_trans (S n1) (S (n1 + n2)) m).
   ...
   + apply H.
 - assert(E : n1 + n2 = n2 + n1).
   + rewrite -> (plus_comm n1 n2). reflexivity.
   + rewrite -> E in H. apply (le_trans (S n2) (S (n2+n1)) m).
     * apply n_le_m__Sn_le_Sm. apply le_plus_l.
     * apply H.
Qed.
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      sentence) explain your answer.  NOPE
...
Lemma plus_eq_one : forall m n, S m = S n -> m = n.
Proof.
  Admitted.

Lemma R_0 : forall n, R 0 n n.
Proof.
  induction n as [|n' IHn'].
  - apply c1.
  - apply (c3 0 n' n'  IHn').
...
Proof.
  intros m n o. split.
  - intros H. induction H as [|m' n_ o' E IH|m_ n' o' E IH|m_ n_ o_ E IH|n_ m_ o_ E IH].
    + unfold fR. reflexivity.
    + unfold fR. unfold fR in IH. rewrite -> (plus_comm n_ (S m')). simpl. 
      rewrite -> (plus_comm m' n_). rewrite -> IH. reflexivity.
    + unfold fR. unfold fR in IH. simpl. rewrite <- IH. reflexivity.
    + unfold fR. unfold fR in IH. simpl in IH. apply plus_eq_one in IH.
      rewrite -> (plus_comm n_ (S m_)) in  IH. simpl in IH. rewrite -> (plus_comm m_ n_) in IH.
      apply plus_eq_one in IH. apply IH.
    + unfold fR. unfold fR in IH. rewrite -> (plus_comm m_ n_) in IH. apply IH. 
  - generalize dependent n. generalize dependent o.
    induction m as [|m' IHm'].
    + intros n o H. unfold fR in H. rewrite -> (plus_comm o 0) in H. simpl in H.
      rewrite -> H. destruct n as [|n'].
      * apply c1.
      * apply (c3 0 n' n'). apply R_0.
    + intros o n H. unfold fR in H. destruct o as [|o'].  
      * rewrite -> (plus_comm n (S m')) in H. discriminate H.
      * apply (c2 m' n o'). unfold fR in IHm'. 
        rewrite -> (plus_comm n (S m')) in H. simpl in H. apply plus_eq_one in H. 
        rewrite -> (plus_comm m' n) in H. apply IHm' in H. apply H.
Qed.
...
Inductive subseq : list nat -> list nat -> Prop :=
  |subseq_c1 : subseq [] []
  |subseq_c2 l1 l2 n (H : subseq l1 l2) : subseq l1 (n :: l2)
  |subseq_c3 l1 l2 n (H : subseq l1 l2) : subseq (n :: l1) (n :: l2).

Theorem subseq_refl : forall (l : list nat), subseq l l.
Proof.
  intros l. induction l as [|h t IHt].
  - apply subseq_c1.
  - apply (subseq_c3 t t h IHt).
Qed.

Lemma subseq_nil_any : forall l, subseq [] l.
Proof.
  intros l. induction l as [|h t IHt].
  - apply subseq_c1.
  - apply (subseq_c2 [] t h IHt).
Qed.

Lemma list_app_first : forall (l1 l2 : list nat) (n : nat),
  (n :: l1) ++ l2 = n :: (l1 ++ l2).
Proof.
  intros l1. destruct l1 as [|h t].
  - intros l2 n. simpl. reflexivity.
  - intros l2 n. simpl. reflexivity.
Qed.
...
Proof.
  intros l1 l2 l3 H.
  induction H as [|l1_ l2' n_ E IH|l1' l2' n_ E IH].
  - simpl. apply subseq_nil_any.
  - rewrite -> list_app_first. apply (subseq_c2 l1_ (l2'++l3) n_ IH).
  - rewrite -> list_app_first. apply (subseq_c3 l1' (l2'++l3) n_ IH).
Qed.
...
Lemma subseq_trans_one : forall (l1 l2 : list nat) (n : nat),
  subseq (n::l1) l2 -> subseq l1 l2.
Proof.
  intros l1 l2. generalize dependent l1. induction l2 as [|h t IH].
  - intros l1 n H. inversion H.
  - intros l1 n H. inversion H as [|l1_ t_ h_ E|l1_ t_ h_ E].
    + apply IH in E. apply (subseq_c2 l1 t h E).
    + apply (subseq_c2 l1 t h E).
Qed.
...
(* induction on H2*)
  ...
  induction H2 as [|l2_ l3' n E IH|l2' l3' n E IH].
  - intros l1 H. apply H.
  - intros l1 H. apply IH in H. apply (subseq_c2 l1 l3' n H).
  - intros l1 H. inversion H as [|l1_ l2'_ n_ H2|l1' l2'_ n_ H2].
    + apply IH in H2. apply (subseq_c2 l1 l3' n H2).
    + apply (subseq_c3 l1' l3' n). apply IH in H2. apply H2.
Qed.
...
Lemma MUnion' : forall T (s : list T) (re1 re2 : @reg_exp T),
  ...
  s =~ Union re1 re2.
Proof.
  intros T s rel1 rel2 [HL | HR].
  - apply (MUnionL s rel1). apply HL. 
  - apply (MUnionR rel1 s). apply HR.
Qed.
...
Proof.
  ...
  - simpl. assert(H2 : In h (h :: t)).
    + simpl. left.  reflexivity.
    + apply H in H2. apply (MStarApp h _ re).
      * apply H2.
      * apply IHt. intros s H3. 
        assert(H4 : In s t -> In s (h::t)).
        { intros H4. simpl. right. apply H3. }
        { apply H4 in H3. apply H in H3. apply H3. }
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. 
    ...
    + destruct H as [s HS]. inversion HS.
    + simpl. reflexivity.
    + simpl. reflexivity.
    + destruct H as [s HS]. simpl. apply andb_true_iff. inversion HS. split.  
      * apply IH1. exists s1. apply H0.
      * apply IH2. exists s2. apply H3.
    + destruct H as [s HS]. simpl. apply orb_true_iff. inversion HS.
      * left. apply IH1. exists s. apply H0.
      * right. apply IH2. exists s. apply H0.
    + simpl. reflexivity.
  - intros H. induction re as [| | t |r1 IH1 r2 IH2| r1 IH1 r2 IH2 | r IH].
    ...
    + simpl in H. apply andb_true_iff in H. destruct H as [H1 H2].
      ...
      exists (s1 ++ s2). apply (MApp s1 r1 s2 r2 HS1 HS2).
    + simpl in H. apply orb_true_iff in H. destruct H as [H1 | H2].
      * apply IH1 in H1. destruct H1 as [s HS]. exists s.
        apply (MUnionL s r1 r2 HS).
      * apply IH2 in H2. destruct H2 as [s HS]. exists s.
        apply (MUnionR r1 s r2 HS).
    + exists []. apply (MStar0 r).
Qed.
(** [] *)
...
(* PAY ATTENTION:
   in circumstances that the Prop on which to perform [induction] is not
   sufficiently general, [induction] tactic fails as is illustrated by the
    instance below 
    
    the key requirment : SUFFICIENTLY GENETAL
*)
...
Proof.
  intros T s re H.
  remember (Star re) as re'.
  induction H as [ | x | s1 rel1 s2 rel2 HM1 IH1 HM2 IH2 
                   | s1 rel1 rel2 HM1 IH1 | rel1 s2 rel2 HM2 IH2
  ...
  - discriminate Heqre'.
  - discriminate Heqre'.
  - injection Heqre' as H. exists []. split.
    + simpl. reflexivity.
    +  intros s'. simpl. intros contra. destruct contra.
  - injection Heqre' as H. 
    assert(HEQ: Star re'' = Star re).
    + rewrite -> H. reflexivity.
    + apply IH2 in HEQ. destruct HEQ as [ss1 HEQ].
      exists ([s1] ++ ss1). split.
      * simpl. destruct HEQ as [HL HR]. rewrite -> HL. reflexivity.
      * intros s'. simpl. intros HOR. destruct HOR as [HL | HR].
        { rewrite <- HL. rewrite <- H. apply HM1. }
        { destruct HEQ as [ _ HRR]. apply HRR in HR. apply HR. }
Qed. 
...
Lemma star_lemma_1 : forall T (l : list T) re,
  l =~ re -> l =~ Star re.
Proof.
  intros T l re H.
  assert(FUCK : l ++ [] = l).
  - apply app_nil_l.
  - rewrite <- FUCK. apply (MStarApp l [] re).
    + apply H.
    + apply (MStar0 re).
Qed. 

Lemma napp_match_star : forall T (m:nat) (l : list T) re,
  l =~ re -> napp m l =~ (Star re).
Proof.
  intros T m l re H.
  induction m as [|m' IH].
  - simpl. apply (MStar0 re).
  - simpl. apply star_app. apply star_lemma_1 in H. apply H.
    apply IH.
Qed.
  

Lemma hd_list : forall T (x : T) (l1 l2 : list T),
  x :: (l1 ++ l2) = (x :: l1) ++ l2.
Proof.
...
Lemma le_plus : forall a b c,   a + b <= c ->
  a <= c /\ b <= c.
Proof.
  intros a. induction a as [|a' IH].
  - intros b c. simpl. intros H. split.
    + apply O_le_n.
    + apply H.
  - intros b c H. destruct c as [|c'].
    + inversion H.
    + simpl in H. apply Sn_le_Sm__n_le_m in H. apply IH in H.
      destruct H as [HL HR]. split.
      * apply n_le_m__Sn_le_Sm. apply HL.
      * apply (le_trans b c' (S c')).
        { apply HR. }
        { apply (le_S c' c' (le_n _)). }
Qed.

Lemma le_test : forall a b c d, a + b <= c + d ->
   a <=c  \/ b <= d.
Proof.
  intros a. induction a as [|a' IH].
  - intros b c d. left. destruct c as [|c'].
    + apply (le_n _).
    + apply le_0_n.
  - intros b c d H. destruct c as [|c'].
    + simpl in H. 
      assert(H2 : a' + b <= d).
      * apply (le_trans (a'+b) (S (a'+b)) d).
        { apply (le_S (a'+b) (a'+b) (le_n (a'+b))). }
        { apply H. }
      * apply le_plus in H2. destruct H2 as [HL HR].
        right. apply HR.
    + simpl in H. apply Sn_le_Sm__n_le_m in H. apply IH in H.
      destruct H as [HL | HR].
      * left. apply n_le_m__Sn_le_Sm. apply HL. 
      * right. apply HR.
Qed.
...
Proof.
  ...
  - (* MApp *)
    ...
    apply le_test in HLen. destruct HLen as [HL | HR].
    + apply IH1 in HL. 
      destruct HL as [s11 [s12 [s13 [H1 [H2 H3]]]]].
      exists s11. exists s12. exists (s13 ++ s2). split.
      * rewrite -> H1. rewrite <- app_assoc. 
        rewrite <- (app_assoc T s12 s13 s2). reflexivity.
      * split.
        { apply H2. }
        { intros m. rewrite -> app_assoc. rewrite -> app_assoc. 
          rewrite <- (app_assoc T s11 (napp m s12) s13).
          apply (MApp (s11 ++ (napp m s12) ++ s13) re1 s2 re2). 
          - apply H3.
          - apply Hmatch2.
        }
   + apply IH2 in HR. 
     destruct HR as [s21 [s22 [s23 [H1 [H2 H3]]]]].
     exists (s1 ++ s21). exists s22. exists s23. split.
     * rewrite <- app_assoc. rewrite -> H1. reflexivity.
     * split.
       { apply H2. }
       { intros m. rewrite <- app_assoc. 
         apply (MApp s1 re1 (s21 ++ napp m s22 ++ s23) re2 Hmatch1).
         apply H3.
       }
 - intros HN. simpl in HN. 
   assert(H1 : pumping_constant re1 <= length s1).
   + destruct (pumping_constant re2) as [|n'].
     * rewrite -> plus_comm in HN. simpl in HN. apply HN.
     * apply (le_trans _ (pumping_constant re1 + S n') (length s1)).
       { apply le_plus_l. }
       { apply HN. } 
   + apply IH in H1.
     destruct H1 as [s21 [s22 [s23 [H1 [H2 H3]]]]].
     exists s21. exists s22. exists s23. split.
     * apply H1.
     * split.
       { apply H2. }
       { intros m. apply (MUnionL _ re1 re2). apply H3. }
 - intros HN. simpl in HN.
   assert(H1 : pumping_constant re2 <= length s2).
   + rewrite plus_comm in HN. apply (le_trans _ (pumping_constant re2 +
     pumping_constant re1) _). apply le_plus_l. apply HN.
   + apply IH in H1. 
     destruct H1 as [s21 [s22 [s23 [H1 [H2 H3]]]]] in H1.
     * exists s21. exists s22. exists s23. split.
       { apply H1. }
       { split. apply H2. intros m. 
         apply (MUnionR re1 _ _). apply H3. }
 - intros H. simpl in H. inversion H.
 - simpl in IH2. intros H. simpl in H. 
   destruct s1 as [|h1 t1].
   + simpl in H. apply IH2 in H. 
     destruct H as [s21 [s22 [s23 [H1 [H2 H3]]]]].
     exists s21. exists s22. exists s23. split.
     * simpl. apply H1.
     * split. 
     { apply H2. }
     { intros m. apply H3. }
  + exists []. exists (h1::t1). exists s2. split.
    * simpl. reflexivity.
    * split.
    { intros contra. discriminate contra. }
    { simpl. intros m. destruct m as [|m'].
      - simpl. apply Hmatch2.
      - simpl. rewrite -> hd_list. rewrite -> (hd_list T h1 t1 _). 
        rewrite <- (app_assoc T (h1::t1) (napp m' (h1::t1)) s2).
        apply (MStarApp (h1::t1) _ ). apply Hmatch1.
        apply (star_app T (napp m' (h1::t1)) s2).
        apply napp_match_star. apply Hmatch1. apply Hmatch2.
    }
Qed.
...
Proof.
  intros P b H. inversion H as [H2 | H2].
  - split. 
    + intros _. reflexivity.
    + intros _. apply H2.
  - split.
    + unfold not in H2. intros HP. apply H2 in HP. destruct HP.
    + intros contra. discriminate contra.
Qed. 
(** [] *)
...
(*ATTENTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  this lemma and the theroem below shows how to use [reflect] to 
  discuss whether a Prop is true or not
  
  learn how to use the techtique
*)
...
Proof.
  ...
  - intros H. intros contra. simpl in contra. destruct contra.
  - intros H. destruct (eqbP n h) as [H1 | H1].
    + rewrite -> H1 in H. simpl in H. rewrite <- eqb_refl in H. simpl in H.
      discriminate H.
    + simpl. intros [HL | HR].
      * rewrite -> HL in H1. apply H1. reflexivity.
      * apply eqb_neq in H1. simpl in H. rewrite -> H1 in H. simpl in H.
        apply IH in H. apply H. apply HR.
Qed.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  |nostutter_c1 : nostutter []
  |nostutter_c2 (x : X) : nostutter [x]
  |nostutter_c3 x h (t : list X) (H1 : nostutter (h::t)) (H2 : x <> h) : nostutter (x::h::t).
...
Inductive listmerge {T} : list T -> list T -> list T -> Prop :=
  | listmerge_c1 l : listmerge [] l l
  | listmerge_c2 l : listmerge l [] l
  | listmerge_c3 x l1 l2 l (H : listmerge l1 l2 l) : listmerge (x::l1) l2 (x::l)
  | listmerge_c4 x l1 l2 l (H : listmerge l1 l2 l) : listmerge l1 (x::l2) (x::l).

Search filter.

Theorem mergeprop : forall T (l1 l2 l : list T) (test : T -> bool),
    (listmerge l1 l2 l) -> filter test l1 = l1 -> filter test l2 = []
    -> filter test l = l1.
Proof.
  intros T l1 l2 l test HP H1 H2.
  induction HP as [l_ | l_ | x l1' l2_ l' E IH| x l1 l2' l' E IH].
  - apply H2.
  - apply H1.
  - assert(HT : test x = true).
    + apply filter_exercise with (x::l1') l1'. apply H1.
    + simpl in H1. rewrite -> HT in H1. injection H1 as H1.  
      apply IH in H1. 
      * simpl. rewrite -> HT. rewrite <- H1. reflexivity.
      * apply H2. 
  - simpl. simpl in H2. destruct (test x) as [] eqn:H3.
    + discriminate H2.
    + apply IH in H1. 
      * apply H1.
      * apply H2.
...
Inductive pal {T} : list T -> Prop :=
  |pal_c1 : pal []
  |pal_c2 (x : T) : pal [x]
  |pal_c3 (x : T) (l : list T) (H: pal l) : pal (x::l ++ [x]).

Theorem pal_app_rev  : forall T (l : list T), pal (l ++ rev l).
Proof.
  intros T l. induction l as [|h t IHt].
  - simpl. apply pal_c1.
  - simpl. rewrite -> app_assoc. apply (pal_c3 h (t ++ rev t)).
    apply IHt.
Qed.

Lemma rev_test : forall T (x : T) (l : list T),
  rev (l ++ [x]) = x :: rev l.
Proof.
  Admitted.
...
Proof.
  ...
  - simpl. reflexivity.  
  - simpl. reflexivity.
  - simpl. rewrite -> rev_test. simpl. rewrite <- IHpal. reflexivity.
Qed.  
...
Theorem rev_length : forall T (l : list T),
  length (rev l) = length l.
Proof.
  intros T l. induction l as [| n l' IHl'].
  - (* l = nil *)
    reflexivity.
  - (* l = cons *)
...
Lemma rev_nil : forall T (l : list T), rev l = [] -> l = [].
Proof.
  intros T l H.
  destruct l as [|h t].
  - reflexivity.
  - assert(H2 : length (h::t) = length (rev (h :: t))).
    + symmetry. apply (rev_length T (h::t)).
    + rewrite -> H in H2. simpl in H2. discriminate H2.
Qed.
...
(* ATTENTION:
  this seems hard to prove in coq


Lemma rev_le : forall T (l : list T), l = rev l -> 
  (exists s, l = s ++ (rev s)) \/ (exists s (x : T), l = s ++ [x] ++ (rev s)).
Proof.
  intros T l H. destruct l as [|h t].
  - left. exists []. simpl. reflexivity.
  - simpl.  
...
Theorem rev_pal : forall T (l : list T), l = rev l -> pal l.
Proof.
  intros T l. z

(*

  intros T l. destruct l as [|h l'].
  - intros _. apply pal_c1.
  - intros H1. simpl in H1. induction (rev l') as [|h' t IH].
    + rewrite -> H1. simpl. apply pal_c2. 
    + apply IH. rewrite -> H1.  
  ...
  - intros H1. simpl in H1. 
    ...
    + assert(H2 : t = []).
      * destruct t as [|h_ t_].
        { reflexivity. }
        { apply rev_nil in E. apply E. }
      * rewrite -> H2. apply pal_c2.
    + simpl in H1.  
